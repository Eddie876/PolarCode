...
int main() {
    ...
    for (int i = 0; i < N; i++) {
        if (Q[i] < 8) {
            Q1[i] = Q[i];
            cout << Q1[i] << " ";  //[0,1,2,4,3,5,6,7]  
        }
    }
    cout << endl;


    //frozen position：Q(1:N-K) //第1個到第N-K個是frozen
    //message position：Q(N-K+1:end)

    //產生K個bit的msg
    cout << "請輸入msg數：";
    cin >> K;
    vector<vector<int>> msg(K);
    ...
    cout << "u[]放入msg後是：";
    for (y = 0; y < 8; y++) {      //最後8換N
        for (int b = 0; b < 4; b++) {  //取位子。只有前4個位子是frozen，在判斷b=0,1,2,3(也就是前N-K個bit)的時候才會跳到63行 4之後要換N-K
            if (y == Q1[b]) {
                c = c + 1;
            }
        }
        ...
        else {                       //後面y=4,5,6,7時，會依序把msg從第0個開始放到u[4,5,6,7]
            u[y] = msg[e];
            e = e + 1;              //下個msg進來就會到下一個ex：u[4]=msg[0]，然後u[5]=msg[1]
        }
        cout << u[y][0] << " ";
        c = 0;
    }
    ...
    while (d > 0) {
        d--;
        int size = pow(2, d);
        vector<vector<int>> newMatrix(size); //size從4變2變1

        for (int i = 0; i < newMatrix.size(); i++) {
            vector<int> arrayLeft = u[2 * i];
            vector<int> arrayRight = u[2 * i + 1];
            vector<int> newArray(arrayLeft.size() + arrayRight.size()); //先宣告一個新的陣列，他的長度變成兩倍
            for (int j = 0; j < arrayLeft.size(); j++) {
                newArray[j] = (arrayLeft[j] + arrayRight[j]) % 2;
                newArray[j + arrayLeft.size()] = arrayRight[j] % 2;
            }
            newMatrix[i] = newArray; //newArray是陣列，把他一個一個丟到newMatrix這個大舉陣中的第i個陣列
        }
        u = newMatrix; //u跟newMatrix都是矩陣，到下一層時，u就會拿上一次的結果去做下一次的運算
    }
    cout << "結果是：";
    for (int p : u[0]) {
        cout << p << " ";
